...
class Room { //lol classroom
    constructor(name, temp = true, playerType = "main") { //wtf else do I add
        this.name = name;
        ...
        this.players = [];
        this.playerSpeed = 1;
        // this.inputs = [false, false, false, false, false]; //mouseDown, mouseUp, mouseMoved, undo. used to check if a room supports a certain input
        this.inputs = {
            mouseDown: false,
            mouseUp: false,
            mouseMoved: false,
        ...
        this.tempRoom = temp; //for when a room is made by a player, and is not a main room.
        this.sendTimer = 0; //timer for how often the server data should be sent to players
        this.maxSendTimer = 1000 / 15;
    }
    ...
    Update(dt, io, id) {
        ...
        for (const p in this.players) {
            ...
            //bits in player.keys associate with directional buttons on keyboard, wasd or arrow keys, which we can decode into up, down, left and right
            let presses = {
                left: (player.keys & 1),
            ...
            list.push({
                id: p,
                position: {
                    x: player.x + player.velocity.x * dt,
                    y: player.y + player.velocity.y * dt
                },
                velocity: {
                    x: player.velocity.x,
                    y: player.velocity.y
            ...
            if (this.boundsCage) {
                if (player.x - 5 < -this.boundSize.width / 2) {
                    player.x = -this.boundSize.width / 2 + 5;
                }
                if (player.y - 5 < -this.boundSize.height / 2) {
                    player.y = -this.boundSize.height / 2 + 5;
                }
                if (player.x + 5 > this.boundSize.width / 2) {
                    player.x = this.boundSize.width / 2 - 5;
                }
        ...
        this.sendTimer += dt;
        if (this.sendTimer > this.maxSendTimer) {
            this.sendTimer -= this.maxSendTimer;
            io.to(id).emit("UpdatePlayers", list);
        }
    }
}
class DrawRoom extends Room { //????????
    constructor(name, temp) {
        super(name, temp, "draw");
        this.inputs = {
            mouseDown: true,
            mouseUp: false,
            mouseMoved: true,
...
class GunRoom extends Room {
    ...
    Update(dt, io, id) {
        super.Update(dt, io, id); //collision between players and walls
...
exports.main = Room;
exports.draw = DrawRoom;
exports.gun = GunRoom;
exports.bullet = Bullet;
exports.bulletTypes = bulletTypes;
exports.gunTypes = gunTypes;